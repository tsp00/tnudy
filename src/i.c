...
Double_t TNuIntg::GetCorrelation(Int_t i, Int_t j) const
{
   ...
   if (i < 0 || i >= GetNNN() || j < 0 || j >= GetNNN()) {
      Error("GetCorrelation", "i=%d, j=%d is out of range, [0, %d)",
         i, j, GetNNN());
      return 0;
   }
...
{
   if (at < 0 || at >= GetNM()) {
      Error("SetAt", "at=%d is out of range, [0, %d)", at, GetNNN());
      return;
   }
   fI[at] = i;
   fJ[at] = j;
   for (Int_t m=0; m < fNRow; m++) fK[at*fNRow + m]= kij[m];
}
