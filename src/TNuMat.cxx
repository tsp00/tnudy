...
void TNuMat::Init(TNuSec01451* pSec1451, TNuLib *lib)
{
   ...
   TNuSublib* fParentSublib = 0;
   fParentSublib = lib->GetSublib(fNSUB);
   SetParent(fParentSublib);
   if (!fParentSublib) {
      // it this sub-library is new,
      // create one, and register it to the library
      fParentSublib = new TNuSublib(fNSUB);
      fParentSublib->SetParent(lib);
      fParentSublib->Init();
      SetParent(fParentSublib);
      lib->Add(fParentSublib);
   }
...
const Char_t* TNuMat::GetProjTargetName(const Char_t* between, Option_t *opt) const
{
   ...
   return Form("%s%s%s",
      //GetParentSublib()->GetShortName(),
      TNuSublib::GetShortName(fNSUB),
      between,
